...
public class Main {
    public static void main(String[] args) {
        ...
        VariableInputPrompt prompt = new VariableInputPrompt();
        boolean isExistingUser = prompt.showInputPrompt(loginPrompt, 1) == 1;
        User user;
        if (isExistingUser) {
        ...
        while (true) {
            ...
            if (choice == topics.length) {
                VariableInputPrompt Prompt = new VariableInputPrompt();
                int response = Prompt.showInputPrompt("Choose one to display:\n(0) Leaderboard\n(1) Statistics", 1);
                if (response == 0) {
            ...
            VariableInputPrompt Prompt = new VariableInputPrompt();
            int mode = Prompt.showInputPrompt("Choose a game mode:\n(0) Random\n(1) Escalation\n(2) Redemption", 2);
            switch (mode) {
                case 0:
                    quiz.askRandom();
                    break;
    ...
    public static int chooseTopic(String[] topics) {
        ...
        VariableInputPrompt prompt = new VariableInputPrompt();
        return prompt.showInputPrompt(builder.toString(), options);
    }
    ...
    public static User promptLogin(Login login) {
        ...
        } while (!login.checkCredentials(user, password));
